...
class ThreeLayerConvNet(object):
  ...
  def loss(self, X, y=None):
    ...
    ############################################################################
    sf_loss, dout = softmax_loss(scores, y)
    loss += sf_loss \
        + 0.5 * self.reg * np.sum(W1 * W1) \
        + 0.5 * self.reg * np.sum(W2 * W2) \
        + 0.5 * self.reg * np.sum(W3 * W3)

    dx, grads['W3'], grads['b3'] = affine_backward(dout, aff2_cache)
    dx, grads['W2'], grads['b2'] = affine_relu_backward(dx, aff_relu_cache)
    dx = max_pool_backward_fast(dx, maxpool_cache)
