...
def batchnorm_backward(dout, cache):
  ...
  dx_hat = dout * gamma
  dvar = -0.5 * \
      np.sum(dx_hat * (x - sample_mean) * np.power((sample_var + eps), -1.5),
             axis=0)
...
def batchnorm_backward_alt(dout, cache):
  ...
  x_corrected = x - sample_mean
  istd = 1 / np.sqrt(sample_var + eps)
  dbeta = np.sum(dout, axis=0)
  dgamma = np.sum(x_hat * dout, axis=0)
  dx = (gamma * istd / N) * (N * dout - np.sum(dout * x_corrected, axis=0) *
                             x_corrected * istd ** 2 - np.sum(dout, axis=0))
      ...
      for f in xrange(F):
          for h_out in xrange(H_out):
              for w_out in xrange(W_out):
                  ...
                  w2 = w_out * stride + WW
                  out[n, f, h_out, w_out] = \
                      np.sum(x_pad[n, :, h1:h2, w1:w2] * w[f, :, :, :]) + b[f]
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  cache = (x, w, b, conv_param)
  ...
  for n in xrange(N):
    for h in xrange(H_prime):
      for w in xrange(W_prime):
        ...
        x_window = x[n, :, h1:h2, w1:w2]
        out[n, :, h, w] = \
            np.max(x_window.reshape((C, pool_height * pool_width)), axis=1)
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  cache = (x, pool_param)
    ...
    for c in xrange(C):
      for h in xrange(H_prime):
        for w in xrange(W_prime):
          ...
          x_window2 = np.reshape(x_window, (pool_height * pool_width))
          x_window3 = np.zeros_like(x_window2)
          x_window3[np.argmax(x_window2)] = 1
          dx[n, c, h1:h2, w1:w2] = \
              np.reshape(x_window3, (pool_height, pool_width)) * dout[n, c, h, w]
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  return dx
